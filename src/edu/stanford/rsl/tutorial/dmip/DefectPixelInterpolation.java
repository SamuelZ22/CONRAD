...
public class DefectPixelInterpolation {
	...
	public Grid2D interpolateSpectral(Grid2D image, Grid2D mask, int maxIter, boolean zeroPadding)
	{
		...
		for(int i = 0; i < maxIter; i++)
		{
			...
			int t2 = (t1>0) ? dim[0]  - (t1 % dim[0]) : t1;//TODO
				...
				(s1 == halfDim[0] && t1 == halfDim[1]))
			{
				...
				//TODO
				//TODO
				Complex Gval = new Complex(G.getRealAtIndex(_s1, t1),G.getImagAtIndex(s1,t1));
				Complex Wval = new Complex(W.getRealAtIndex(_0, 0),W.getImagAtIndex(0, 0));
				Complex FHatNextVal = new Complex(FHatNext.getAtIndex(s1, t1),FHatNext)
			...
			else
			{
				...
				Complex WVal_twice = new Complex(W.getRealAtIndex(twice_s1_, twice_t1),G.getImagAtIndex(twice_s1, twice_2));
				
				Complex FHatNextVal_s1t1 = new Complex(FHatNext(s1,t1), FHatNext(s1,t1));
				Complex FHatNextVal_s2t2 = new Complex()
				
				Complex val = ((GVal.mul(WVal_00.getConjugate()).sub(GVal.getConjugate().mul(WVal_twice))).mul(dim[0]*dim[1]);
				val = val.div(WVal_00).getMagn() * WVal_00.getMagn() - WVal_twice.getMagn());
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/hy83xehu/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/testimg.bmp";
		String filenameMask = "/proj/i5dmip/hy83xehu/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/mask.bmp";
